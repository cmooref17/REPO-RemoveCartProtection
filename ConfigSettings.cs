...
namespace REPO_RemoveCartProtection.Config
{
    ...
    public static class ConfigSettings
    {
        ...
        internal static void BindConfigSettings()
        {
            ...
            keepImpactProtectionExtraction = AddConfigEntry(Plugin.instance.Config.Bind("General", "Keep Impact Protection In Extraction", true, new ConfigDescription("[Host-only] Keeps the vanilla impact protection from objects in the extraction zone.")));
            removeProtectionBlacklist = AddConfigEntry(Plugin.instance.Config.Bind("General", "Remove Impact Protection - Blacklist", "Music Box,Frog,Propane Tank,Wizard Staff,Scream Doll,Saw Blade,", new ConfigDescription("[Host-only] Add item names here to prevent removing impact protection from. Separate each item with a \",\" character. Example: \"ItemName1,ItemName2\"\nItems in this blacklist will behaving normally when placing them into the cart.\nNOTE: Item names are a little funky to reference, so at this time, the formatting will be generous and may not need to match 100% of the item name, but may have issues in strange scenarios. Will refine in the future.")));
        ...
        internal static string[] ParseBlacklistedItemNames()
        {
            ...
            {
                Plugin.LogError("Error parsing impact protection blacklist.\n" + e);
                return new string[0];
            }
        }
    }
